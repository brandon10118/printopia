        ...
        <div id="section-overview" class="dashboard-section">
            ...
            <div id="section-storage" class="dashboard-section" style="display: none;">
                <div class="card border-0 shadow-sm p-4">
                    ...
                    <form id="cloudinary-upload-form" class="mb-4">
                        <div class="d-flex gap-2">
                            <input type="file" id="user_file" class="form-control form-control-sm"
                                style="max-width: 300px;" required>
                            <button type="submit" class="btn btn-primary btn-sm">Upload</button>
                        </div>
                    </form>

                    <h6 class="text-muted small fw-bold mb-3 uppercase">Folders</h6>
                    <div id="folder-container" class="row g-3 mb-5">
                    </div>

                    <hr>

                    <h6 class="text-muted small fw-bold mb-3 uppercase">Files</h6>
                    <div id="file-container" class="row g-3">
                    </div>
                </div>
            </div>
                    ...
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body p-4">
                            ...
                            <a href="#" class="btn btn-link text-decoration-none small p-0 mt-3"
                                style="color: #4A70A9;">View all orders →</a>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="progress mb-3" style="height: 12px; border-radius: 10px; background: #e9ecef;">
                                <div class="progress-bar" role="progressbar"
                                    style="width: 32%; background: linear-gradient(90deg, #8FABDA 0%, #4A70A9 100%); border-radius: 10px;"
                            ...
                            <div class="d-flex flex-column gap-2">
                                <div class="d-flex align-items-center justify-content-between">
                                    <div class="d-flex align-items-center gap-2">
                                        <div
                                            style="width:32px; height:32px; background: rgba(74,112,169,0.06); border-radius:6px; display:flex; align-items:center; justify-content:center;">
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#4A70A9"
                                                stroke-width="2">
                                ...
                                <div class="d-flex align-items-center justify-content-between">
                                    <div class="d-flex align-items-center gap-2">
                                        <div
                                            style="width:32px; height:32px; background: rgba(139,171,218,0.06); border-radius:6px; display:flex; align-items:center; justify-content:center;">
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#8FABDA"
                                                stroke-width="2">
                                                <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" />
                                                <polyline points="14 2 14 8 20 8" />
                                            </svg>
            ...
            <div id="section-history" class="dashboard-section" style="display: none;">
                ...
                <div class="d-flex align-items-center gap-2 mb-3 mt-3">
                    ...
                    <span class="ms-auto badge rounded-pill" style="background: #e0e0e0; color: #666;">Last 30
                        days</span>
                </div>
...
<script>
    ...
    // Loading state
    folderContainer.innerHTML = '<p class="small text-muted">Loading...</p>';
    ...
    try {
        ...
        // 1. Render Folders (Top)
        if (data.folders && data.folders.length > 0) {
            data.folders.forEach(f => {
                folderContainer.innerHTML += `
                    <div class="col-md-3 col-6">
                        <div class="p-3 border rounded shadow-sm d-flex align-items-center gap-2 cursor-pointer bg-light">
                            <span class="h4 mb-0">📁</span>
                            <span class="small fw-bold text-truncate">${f.name}</span>
                        </div>
                    </div>`;
            });
        } else {
            folderContainer.innerHTML = '<p class="small text-muted ms-2">No folders found.</p>';
        }

        // 2. Render Files (Bottom)
        if (data.files && data.files.length > 0) {
            data.files.forEach(file => {
                fileContainer.innerHTML += `
                    <div class="col-md-2 col-6">
                        <div class="card border-0 shadow-sm h-100">
                            <img src="${file.secure_url}" class="card-img-top object-fit-cover" style="height: 100px;">
                            <div class="p-2 border-top">
                                <p class="small text-truncate mb-0" title="${file.public_id}">${file.public_id.split('/').pop()}</p>
                            </div>
                        </div>
                    </div>`;
            });
        } else {
            fileContainer.innerHTML = '<p class="small text-muted ms-2">No files found.</p>';
        }
    } catch (err) {
